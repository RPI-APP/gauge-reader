...
/**
 * This is a copy of com.github.sarxos.webcam.WebcamPanel. Changes were marked with // dfc
 * TODO: Extend the original more cleanly?
 * 
 ...
 */
public class BetterWebcamPanel extends JPanel implements WebcamListener, PropertyChangeListener {
	
	// dfc
	private volatile BufferedImage latestImage = null;
	private volatile int resizeX = -1;
	private volatile int resizeY = -1;
	private volatile int resizeW = -1;
	private volatile int resizeH = -1;
	...
	public static interface Painter {
		 ...
		 */
		void paintPanel(BetterWebcamPanel panel, Graphics2D g2);
		 ...
		 * @param g2 the graphics 2D object used for drawing
		 */
		void paintImage(BetterWebcamPanel panel, BufferedImage image, Graphics2D g2);
	}
	...
	public class DefaultPainter implements Painter {
		...
		@Override
		public void paintPanel(BetterWebcamPanel owner, Graphics2D g2) {
		...
		@Override
		public void paintImage(BetterWebcamPanel owner, BufferedImage image, Graphics2D g2) {
			...
			// dfc
			BetterWebcamPanel.this.latestImage = image;
			...
			// dfc
			BetterWebcamPanel.this.resizeX = x;
			BetterWebcamPanel.this.resizeY = y;
			BetterWebcamPanel.this.resizeW = w;
			BetterWebcamPanel.this.resizeH = h;
			...
			if (isDisplayDebugInfo()) {
				...
				if (lastRepaintTime < 0) {
					...
					for (String line : res.split("\n")) {
						g2.setColor(Color.BLACK);
						g2.drawString(line, strx, stry);
						g2.setColor(Color.WHITE);
						g2.drawString(line, strx - 1, stry - 1);
						stry += g2.getFontMetrics().getHeight();
				    }
				}
			}
	...
	private static final class SwingRepainter implements Runnable {
		...
		private BetterWebcamPanel panel = null;

		public SwingRepainter(BetterWebcamPanel panel) {
			this.panel = panel;
		}
	 ...
	 */
	public BetterWebcamPanel(Webcam webcam) {
		this(webcam, true);
	}
	 ...
	 */
	public BetterWebcamPanel(Webcam webcam, boolean start) {
	 ...
	 */
	public BetterWebcamPanel(Webcam webcam, Dimension size, boolean start) {
	...
	// dfc
	/**
	 * Converts an x-y coordinate on the screen to the x-y on the original image
	 * @param x
	 * @param y
	 */
	public Point2D.Double screenToRaw(Point2D.Double p)
	{
		BufferedImage full = this.latestImage;
		if (full == null) {
			return null;
		}
		...
		double rx = this.resizeX;
		double ry = this.resizeY;
		double rw = this.resizeW;
		double rh = this.resizeH;
		
		double iw = full.getWidth();
		double ih = full.getHeight();
		
		double px = (x - rx) / rw * iw;
		double py = (y - ry) / rh * ih;
	...
	//dfc
	public Point2D.Double rawToScreen(Point2D.Double p)
	{
		BufferedImage full = this.latestImage;
		if (full == null) {
			return null;
		...
		double px = x / iw * rw + rx;
		double py = y / ih * rh + ry;
